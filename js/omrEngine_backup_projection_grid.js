...
const OmrEngine = {
    ...
    findGrid(grayData, width, height) {
        ...
        this._log(`[투영감지] X피크=${colPeaks.length}개 [${colPeaks.map(p=>Math.round(p.pos)).join(',')}]`);
        this._log(`[투영감지] Y피크=${rowPeaks.length}개 [${rowPeaks.map(p=>Math.round(p.pos)).join(',')}]`);
    ...
    // 메인 분석
    // ==========================================
    analyzeROI(imageData, offsetX, offsetY, orientation = 'vertical', numQ = 0, numC = 0, savedGrid = null) {
        ...
        if (savedGrid && savedGrid.qPositions && savedGrid.cPositions) {
            // ★ 양식에서 저장된 그리드 사용 (투영 감지 건너뜀)
            qPositions = [...savedGrid.qPositions];
            cPositions = [...savedGrid.cPositions];
            numQ = qPositions.length;
            numC = cPositions.length;
            this._log(`[양식그리드] 저장된 좌표 사용: ${numQ}문항 × ${numC}지선다`);
        } else {
            // 투영 프로파일로 그리드 감지
            ...
            if (grid) {
                ...
                this._log(`[균등분할] 투영 감지 실패 → ${numQ}×${numC} 균등 분할`);
            }
        }
        ...
        this._log(`[그리드분석] ${numQ}문항 × ${numC}지선다, ${isVert ? '세로' : '가로'}, Q위치=[${qPositions.map(p => Math.round(p)).join(',')}], C위치=[${cPositions.map(p => Math.round(p)).join(',')}]`);
        ...
        for (let q = 0; q < numQ; q++) {
            ...
            if (bestIdx !== -1 && numC > 1) {
                ...
                // 백지 판별: 최대 fill이 낮고 fill 차이가 작으면 → 아무것도 안 칠해짐
                ...
                if (!isBlank && promRatio < 3 && bestFill < 0.9) {
                    ...
                    this._log(`    → Q${q + 1} 2차검증: ${nScoreLog} prom=${nPromRatio.toFixed(2)} (1차: ${primaryMarked !== -1 ? primaryMarked + 1 : 'null'})`);

                    if (nPromRatio > 1.5) {
                        if (primaryMarked === -1 || nBestIdx !== primaryMarked) {
                            primaryMarked = nBestIdx;
                            this._log(`    → Q${q + 1} 2차 채택: answer=${primaryMarked + 1}`);
                        }
                    } else if (primaryMarked !== -1) {
                        this._log(`    → Q${q + 1} 2차 불확실: 미기입 처리`);
                        primaryMarked = -1;
                    }
                }
        ...
        return { rows: structuredRows, maxCols: numC, debugBlobs, gridData };
    },
    ...
    autoDetect(imageData, offsetX, offsetY) {
        ...
        this._log(`[자동감지] 투영 → 방향=${orientation} 문항=${numQuestions} 지선다=${numChoices}`);
