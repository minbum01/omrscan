...
const CanvasManager = {
    ...
    initAdjustBar() {
        ...
        // 버블 크기 슬라이더 (debounce로 실시간 동기화)
        const bubbleInput = document.getElementById('adj-bubble-size');
        const bubbleVal = document.getElementById('adj-bubble-size-val');
        let bubbleDebounce = null;
        bubbleInput.addEventListener('input', () => {
            this.bubbleSize = parseInt(bubbleInput.value);
            bubbleVal.textContent = this.bubbleSize === 0 ? '자동' : this.bubbleSize + 'px';
            clearTimeout(bubbleDebounce);
            bubbleDebounce = setTimeout(() => {
                this.runAnalysis();
            }, 300);
        });
