...
const OmrEngine = {
    ...
    analyzeROI(imageData, offsetX, offsetY, orientation = 'vertical', numQ = 0, numC = 0) {
        ...
        // ROI 범위 클램핑
        qPositions = qPositions.filter(p => p >= 0 && p < (isVert ? height : width));
        cPositions = cPositions.filter(p => p >= 0 && p < (isVert ? width : height));

        // 부족 시 다시 보간
        if (qPositions.length < numQ) qPositions = this.adjustPositions(qPositions, numQ, false);
        if (cPositions.length < numC) cPositions = this.adjustPositions(cPositions, numC, isVert);
        ...
        for (let q = 0; q < numQ; q++) {
            ...
            if (primaryMarked !== -1 && numC > 1) {
                ...
                cellScores.forEach((c,i) => {
                    const fD = c.centerFill - avgFill, iD = c.darkRatio - avgInk;
                    if (c.score > pS * 0.9 && (fD > 0.12 || iD > 0.08)) markedIndices.push(i);
                });
                if (!markedIndices.includes(primaryMarked)) markedIndices.unshift(primaryMarked);
            } else if (primaryMarked !== -1) markedIndices.push(primaryMarked);
            ...
            console.log(`  Q${q+1}: ${cellScores.map(c => `[${c.col+1}] s=${Math.round(c.score)} b=${Math.round(c.brightness)} f=${c.centerFill.toFixed(3)}`).join(' | ')} → answer=${finalMarked}${isMulti ? ' (중복)' : ''} prom=${promRatio.toFixed(2)}`);
        ...
        return { rows: structuredRows, maxCols: numC };
    },
