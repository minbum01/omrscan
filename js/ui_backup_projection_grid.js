...
const UI = {
        ...
        if (imgObj.rois.length === 0) {
            ...
            imgObj.rois.forEach((roi, idx) => {
                ...
                html += `<div class="roi-card-header">
                    ...
                    ${s.grid ? '<span style="color:#22c55e;font-size:10px;font-weight:700;margin-left:4px;">양식저장됨</span>' : ''}
                ...
                html += `<div style="padding:4px 8px; border-top:1px solid var(--border-light);">
                    ...
                    ${s.grid ? `<button class="btn btn-sm" style="width:100%; font-size:11px; padding:4px; margin-top:2px; color:var(--red);"
                        onclick="UI.clearGrid(${idx})">그리드 초기화</button>` : ''}
                </div>`;
    ...
    setOrientation(roiIdx, value) {
        ...
        const oldOrient = s.orientation;

        if (oldOrient !== value) {
            // 방향 변경 시 문항수와 지선다 스왑
            const tmpQ = s.numQuestions;
            s.numQuestions = s.numChoices;
            s.numChoices = tmpQ;

            // 선택지 프리셋도 갱신
            if (value === 'horizontal' && s.numChoices >= 9) {
                s.choicePreset = '1-0';
                s.choiceLabels = [...UI.CHOICE_PRESETS['1-0'].labels];
                s.numChoices = 10;
            } else {
                const presetMap = { 4: '1-4', 5: '1-5', 9: '1-9', 10: '1-0' };
                if (presetMap[s.numChoices]) {
                    s.choicePreset = presetMap[s.numChoices];
                    s.choiceLabels = [...UI.CHOICE_PRESETS[s.choicePreset].labels];
                } else {
                    s.choiceLabels = Array.from({ length: s.numChoices }, (_, i) => String(i + 1));
                }
            }
        }
    ...
    onCellInput(input) {
        ...
        if (val === '0' || val === '-') {
            ...
            else { const n = parseInt(val); if (n >= 1 && n <= numC) newAnswer = n; }
        } else {
            const n = parseInt(val); if (n >= 1 && n <= numC) newAnswer = n;
        }
